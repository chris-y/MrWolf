...
DEF clkioreq:timerequest,clkmsgport:mp,lssecs=0
...
DEF curtime,tzname1[3]:STRING,tzname2[4]:STRING,dst=0,utc=1,timeint=0
...
DEF ttnotify:notifyrequest,ttnotifysig,ttnotifyport:mp
...
verstring:='$VER: FACTS 4.10 (25.09.2002)' /* internal 4.10, release 4.91 */
fver:='4'
frev:='10'
frel:='4.91'
...
IF wbmessage=NIL
    ...
    WriteF('© 1998-2002 Unsatisfactory Software\n\n')
ENDIF
...
ELSE
    StrCopy(server,'tick.usno.navy.mil')
ENDIF
*/
...
DEF datestring:clockdata,tmp2[100]:STRING
...
IF win:=OpenWindowTagList(NIL,[WA_LEFT,winx,
                            ...
                            WA_SCREENTITLE,'FACTS  © 1998-2002 Unsatisfactory Software',
                            WA_AUTOADJUST,TRUE,
                            WA_ZOOM,zoompos,
                            NIL])
...
DEF signal,msgid,msgtype,msg,clgui=0,tisig,oldtz,clksig,tmpint
...
signal:=Wait(winsig OR cxsigflag OR tisig OR arexxsig OR notifysig OR ttnotifysig OR appmenusig OR dstsig OR clksig)
      ...
      CASE IDCMP_MENUPICK
        ...
        WHILE(menunumber<>MENUNULL)
            ...
            IF menunum=0 -> Project
              ...
              IF itemnum=3 THEN about()
              IF itemnum=5 THEN clgui:=1
              IF itemnum=6 THEN terminated:=TRUE
/*
                IF EasyRequestArgs(0,[SIZEOF easystruct,0,'FACTS',locstr[MSG_QUIT],yesno],0,NIL) THEN terminated:=TRUE
              ENDIF
*/
        ...
        REPEAT
            ...
            IF arexxmsg<>NIL
                ...
                IF StrCmp(rexxcmd,'COPYRIGHT')
                    StrCopy(result,'© 1998-2002 Unsatisfactory Software')
                ENDIF
                ...
                IF StrCmp(rexxcmd,'DSTMODE',7)
                    IF EstrLen(rexxcmd)>8
                        dstswitch(Val(MidStr(temp,rexxcmd,7)),1)
                    ELSE
                        StringF(result,'\d',dst)
                    ENDIF
                ENDIF

                IF StrCmp(rexxcmd,'DST')
                    StringF(result,'\d',dst)
                ENDIF
...
PROC saveconfig()
DEF olddir=-1,oldtooltypes,newtooltypes[45]:ARRAY OF LONG,tmptt1[50]:STRING
...
menustrip:=CreateMenusA([NM_TITLE,0,locstr[MENU_PROJECT],0,0,0,0,
                         ...
                         NM_SUB,0,'Canada/Newfoundland/Mexico',0,autodstmen[5],2097119,0,
                         ...
                         NM_SUB,0,'United States of America',0,autodstmen[18],1835007,0,
...
StrAdd(abstr,'\nARexx Port: FACTS\nStratum 1 Data: ')
StrAdd(abstr,getdataver(1))
StrAdd(abstr,'\nStratum 2 Data: ')
StrAdd(abstr,getdataver(2))
StrAdd(abstr,'\nTCP/IP: ')
...
IF service=123
    ioreq.io.command:=TR_GETSYSTIME
    DoIO(ioreq)
    dess:=ioreq.time.secs+2461449600-tzsecs
->    desf:=ioreq.time.micro*4295

    -> local clock offset 
->    difference := ((pkt.recs-pkt.orgs)+(pkt.xmts-dess))/2
/*
    WriteF('clkoff: \d ',clkoff)
    clkoff := ((pkt.recf-pkt.orgf)+(pkt.xmtf-desf))/2
    WriteF('f \d\n',UNSIGNED(clkoff))
*/
    -> roundtrip delay
    rtrips := (dess-pkt.orgs) - (pkt.recs-pkt.xmts)
->    WriteF('roundtrip: \d ',rtrips)
...
/*
    ...
    xmtf:LONG
*/
    -> servsecs:=Long(buffer[40])-2461449600
    servsecs:=pkt.xmts-2461449600+rtrips

/* note: following is not adjusted for roundtrip delay */
-> ntpfrac:=Mul(pkt.xmtf!,2.0)

servmicros:=Shr(Mul(pkt.xmtf,1000000),12)+500000+rtrips

IF servmicros>1000000
    servmicros:=1000000
ENDIF
...
/*
StrAdd(info,'\nStratum: ')
StrAdd(info,RealF(temp,pkt.stratum!,0))
*/
...
/*
    StrAdd(info,'\nReference: ')
    StrAdd(info,RealF(temp,pkt.refid[0]!,0))
    StrAdd(info,'.')
    StrAdd(info,RealF(temp,pkt.refid[1]!,0))
    StrAdd(info,'.')
    StrAdd(info,RealF(temp,pkt.refid[2]!,0))
    StrAdd(info,'.')
    StrAdd(info,RealF(temp,pkt.refid[3]!,0))
*/
ELSE
...
/*
StrAdd(info,'\nPoll Interval: ')
StrAdd(info,RealF(temp,pkt.poll!,0))
*/
...
/*
StrAdd(info,'\nPrecision: ')
StrAdd(info,RealF(temp,(pkt.precision-256)!,0))
*/
...
/*
StrAdd(info,'\nRoot Delay: ')
StrAdd(info,RealF(temp,pkt.rootdelay!,0))
StrAdd(info,'.')
StrAdd(info,RealF(temp,pkt.rootdelayf!,0))
*/
...
/*
StrAdd(info,'\nRoot Dispersion: ')
StrAdd(info,RealF(temp,pkt.rootdisp!,0))
StrAdd(info,'.')
StrAdd(info,RealF(temp,pkt.rootdispf!,0))
*/
...
ENDIF
    StringF(temp,'\s|\s',locstr[MSG_OK],locstr[MSG_INFO_PUBLISHED])
...
SELECT autodst
    ...
    CASE 5 -> Canada/Thule/Newfoundland/Mexico/Bahamas
    startmonth:=4
    startwday:=0
    starthour:=2
    startweek:=1
