...
verstring:='$VER: FACTS 5.7 (31.03.2007)' /* internal 5.1, release 5.0 */
fver:='5'
frev:='7'
frel:='5.06beta'
...
******* temp query test ends ****/
    ...
    pkt.recs:=0
    pkt.recf:=0
    pkt.xmts:=measure.secs+0-tzsecs -> 2461449600 **** portablE ****
...
IF battclockbase:=OpenResource(BATTCLOCKNAME)
    ...
    IF batt2078
        ioreq.time.secs:=ioreq.time.secs+0 -> **** portable **** 3155760000
    ENDIF
    ioreq.io.command:=TR_SETSYSTIME
    DoIO(ioreq)
ELSE
...
IF service=123
    ...
    /* ioreq.io.command:=TR_GETSYSTIME
    DoIO(ioreq) */
    dess:=measure.secs+0-tzsecs -> **** portable **** 2461449600
    ...
    servsecs:=pkt.xmts-0+rtrips -> 2461449600 ****portable ****
...
ELSE
servsecs:=Long(buffer)-0 /* num secs from  1 Jan 1900 00:00:00
