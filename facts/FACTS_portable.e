...
-> PortablE version closes immediately after open (time signal?)
...
DEF ioreq:PTR TO timerequest,msgport:PTR TO mp,win:PTR TO window,pixsep=5,wbstart=1
DEF clkioreq:PTR TO timerequest,clkmsgport:PTR TO mp,lssecs=0,envioreq:PTR TO timerequest,envmsgport:PTR TO mp
DEF gads[31]:ARRAY OF LONG,mysc:PTR TO screen,vi,tbar,utcoffset=3600
DEF glist:PTR TO LONG,cxpri=0,popkey[256]:STRING,winopen=0,tioreq:PTR TO timerequest,nullmsgport:PTR TO mp
...
DEF measure:PTR TO timeval,timemem:PTR TO timeval,response,arexxport -> timerbase:PTR TO lib,
DEF donotopenwin=0,winsig=NIL,filter2,savekey[256]:STRING,autodst=0
DEF tznotify:notifyrequest,notifysig,notifyport:PTR TO mp,ignoretz=0,port=123,service=123
DEF ttnotify:notifyrequest,ttnotifysig,ttnotifyport:PTR TO mp,smeasure:PTR TO timeval
...
DEF dirlock=0,dstioreq:PTR TO timerequest,dstmsgport:PTR TO mp,socketbase:PTR TO lib,defport=1
...
ENUM    ERR_MEM,ERR_NOHOST,ERR_NOTCP,ERR_NOCLK,ERR_DEV,ERR_UTIL,
...
verstring:='$VER: FACTS 5.8 (13.09.2009)' /* internal 5.1, release 5.0 */
StringF(fver,'5')
StringF(frev,'8')
StringF(frel,'5.06')
...
IF wbmessage=NIL
    ...
    WriteF('© 1998-2009 Unsatisfactory Software\n\n')
ENDIF
...
ENDFOR
    ...
    StringFL(monthnames,'   \s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]\s[3]',[GetLocaleStr(localeinfo,ABMON_1),
...
******* temp query test ends ****/
    ...
    pkt.recs:=0
    pkt.recf:=0
    pkt.xmts:=measure.secs + $92B6BD80 - tzsecs
...
IF battclockbase:=OpenResource(BATTCLOCKNAME)
    ...
    IF batt2078
        ioreq.time.secs:=ioreq.time.secs+ $BC191380
    ENDIF
    ioreq.io.command:=TR_SETSYSTIME
    DoIO(ioreq)
ELSE
...
IF win:=OpenWindowTagList(NIL,[WA_LEFT,winx,
                            ...
                            WA_IDCMP,IDCMP_CLOSEWINDOW OR IDCMP_REFRESHWINDOW OR BUTTONIDCMP OR IDCMP_MENUPICK OR IDCMP_NEWSIZE,
                            WA_GADGETS,glist,
                            WA_TITLE,defwintit,
                            WA_DRAGBAR,TRUE,
                            WA_DEPTHGADGET,TRUE,
                            ...
                            WA_SCREENTITLE,'FACTS  © 1998-2009 Unsatisfactory Software',
                            WA_AUTOADJUST,TRUE,
                            WA_ZOOM,zoompos,
                            NIL])
...
-> 270
  ...
  gads[GADG_DST]:=CreateGadgetA(CHECKBOX_KIND, gads[GADG_TZ],
                    [((StrLen(locstr[LAB_GADG_TZNAME])+StrLen(locstr[LAB_GADG_TZ])+StrLen(locstr[LAB_GADG_DST])+10)*txtxs)+30, (pixsep*5)+tbar+(mysc.font.ysize*4),
...
-> 300
  ...
  gads[GADG_DSTNAME]:=CreateGadgetA(STRING_KIND, gads[GADG_DST],
                    [((StrLen(locstr[LAB_GADG_TZNAME])+StrLen(locstr[LAB_GADG_TZ])+StrLen(locstr[LAB_GADG_DST])+10)*txtxs)+42+mysc.font.ysize,
(pixsep*5)+tbar+(mysc.font.ysize*4),
...
bottomlinesize:=((StrLen(locstr[LAB_GADG_TZNAME])+StrLen(locstr[LAB_GADG_TZ])+StrLen(locstr[LAB_GADG_DST])+16)*txtxs)+47+mysc.font.ysize

bottomlinesize:=Max(bottomlinesize,intsize+210+(txtxs*4))
bottomlinesize:=Max(bottomlinesize,maxi+210+butsize)

widthts:=bottomlinesize-Max(intsize+210+(txtxs*4),maxi+205+butsize)
        ...
        REPEAT
            ...
            IF arexxmsg<>NIL
                ...
                IF StrCmp(rexxcmd,'COPYRIGHT')
                    StrCopy(result,'© 1998-2009 Unsatisfactory Software')
                ENDIF
...
IF stgemul
 ...
 StringFL(stgstr,'NT:\d:\d:\d:\d:\d:\d:\d:\d:\d:\d:',[stgtz,startweek,startwday,startmonth,(starthour*60),endweek,endwday,endmonth,(endhour*60),stgtz+60])
ENDIF
...
IF service=123
    ...
    /* ioreq.io.command:=TR_GETSYSTIME
    DoIO(ioreq) */
    dess:=measure.secs+$92B6BD80-tzsecs
    ...
    servsecs:=pkt.xmts-$92B6BD80+rtrips
...
ELSE
servsecs:=Long(buffer)-$92B6BD80 /* num secs from  1 Jan 1900 00:00:00
...
IF leapsec=1 THEN dodefop(curtime+1)
IF leapsec=2 THEN dodefop(curtime-1)
